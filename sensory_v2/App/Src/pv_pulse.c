...
int get_pulse( uint16_t* adc_buf, uint16_t arr_size) {
    ...
    uint16_t samples_per_period[32] = {0};
...
int main() {
    ...
    uint16_t arr[16]= { 5,5,1,1,5,5,1,1,5,5,1,1,5,5,1,1};
    int res = get_pulse(arr, sizeof(arr)/sizeof(arr[0]));
    printf("\n\nInput: "); for( int i=0; i<(sizeof(arr)/sizeof(arr[0])); i++ ) printf("%d, ", arr[i]);
    printf("\nResult: %d\n", res);
